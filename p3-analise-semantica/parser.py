...
def p_declaracao_variaveis_error(p):
    ...
    """
    print('Erro na regra: declaracao_variaveis')
...
def p_lista_variaveis_error(p):
    ...
    """
    print("Erro na regra: lista_variaveis")
...
def p_declaracao_funcao_error(p):
    ...
    """
    print("Erro na regra: declaracao_funcao")
...
def p_lista_parametros_error(p):
    """lista_parametros : lista_parametros VIRGULA error
    """
...
def p_expressao_logica_error(p):
    ...
    """
    print("Erro na regra: expressao_logica")
...
def p_expressao_simples_error(p):
    """expressao_simples : expressao_simples operador_relacional error
                        | error operador_relacional expressao_aditiva
                        | expressao_simples error expressao_aditiva
    """
    print("Erro na regra: expressao_simples")
...
def p_expressao_aditiva_error(p):
    """expressao_aditiva : expressao_aditiva operador_soma error
                        | error operador_soma expressao_multiplicativa
    """
    print("Erro na regra: expressao_aditiva")
...
def p_expressao_multiplicativa_error(p):
    """expressao_multiplicativa : expressao_multiplicativa operador_multiplicacao error
                                | error operador_multiplicacao expressao_unaria
    """
    print("Erro na regra: expressao_multiplicativa")
...
def p_expressao_unaria_error(p):
    """expressao_unaria : operador_soma error
                        | operador_negacao error
    """
    print("Erro na regra: expressao_unaria")
...
def p_chamada_funcao_error(p):
    ...
    """
    print("Erro na regra: chamada_funcao")
...
def p_lista_argumentos_error(p):
    """lista_argumentos : lista_argumentos VIRGULA error
    """
    print("Erro na regra: lista_argumentos")
...
def p_error(p):
    ...
    if p:
        token = p
        print("Erro [Linha {line}]: Erro próximo ao token '{token}'".format(
...
def main():
    aux = argv[1].split('.')
    if aux[-1] != 'tpp':
      raise IOError("Não é um arquivo com extensão .tpp!")
    data = open(argv[1])
    ...
    if root and root.children != ():
        ...
        print("Grafo gerado.\nArquivo disponível em: " + argv[1] + ".ast.png")
        ...
        print("Não é possível gerar a árvore sintática.")
    print('\n\n')
